  ...
  */
     ...
     // bolden text
	   ...
	   // set system name {lable}
		 $('.systemLabel').append('School System');
      ...
      $( ".hm-btn-modal-e1" ).click( () => {
         // hilmacs default comment
				 var x = ({
					 ...
					 time:$('.exam-time').val(),
           subjects:$('.hm-inpt-modal-c2').val().split(","),
			   });
				 if (x.name && x.abbr) {
           $.post(hmPath.gate('exams',null,'data/'), { 'a' : x.name , 'b': x.abbr,'subjects':x.subjects, 'd':x.time } ).done( () => {
           	hmService.hapiExams();
           });
				 } else {
           alert(hmLang.locals('noChg'));
     ...
     $( ".hm-btn-modal-c1" ).click( () => {
         // hilmacs default comment
				 ...
				 if (x.name) {
           $.post(hmPath.gate('classes',null,'data/'), { 'a' : x.name , 'b': x.stream,'subjects':x.subjects } ).done( () =>{
     ...
     hmService.previewQuestion = (question) =>{
       $('.question-preview').empty()
       $('.question-preview').append(`
            <div class="row">
              <div class="">
            ...
            <div class="row">
              ${question.answers.map((answer)=> `<div class="card col-sm-4">${answer}</div>`)}
            </div>
            <div class="row">
              <h4>Main Answer</h4>
              <p>${question.answer}</p>
     ...
     hmService.getAnswers = async (query )=>{
      //let answers = await $.post("http://url",{query:query.question}).done((answers)=>{
        console.log("getAnswers =>",query)
      //})
      if (query) {
        $.put(hmPath.gate('questions',null,'data/answers'), { 'id':query._id,'answers':[] } ).done( (data)=> {
     ...
     $( ".add-question" ).click( () => {
      ...
      console.log(x)
      if (x.question) {
        $.post(hmPath.gate('questions',null,'data'),{'exam':x.exam,'subject':x.subject,'question':x.question,
              'type':x.type,
              'value':x.value,
              'category':x.category,
              'answer':x.answer
     ...
     $( ".hm-nav-bar-top-sandwich" ).click( () => {
        // hilmacs default comment
		 ...
		 // hilmacs default comment
		 // hilmacs default comment
		 $(".hm-inpt-search-top-0").on("submit", function(e){
